...
def zip2dict(zip_filename, students):
    ...
    for folder in folder_list:
        student_id = folder.split("/")[-1][:8]
        student_ids.append(student_id)
        #print(student_id)
        if folder[-4:] != ".zip":
            continue
        with zipfile.ZipFile(folder) as extract_zip:
            extract_zip.extractall('tmp/'+student_id)

    for student_id in student_ids:
        # 2019/10/18
        # おそらく，glob.glob("tmp/" + student_id + "/**) の方が良い
        # ただ，フォルダかどうかos.path.isdirのチェックが入らない事に注意
        for filename in glob.glob("tmp/" + student_id + "/*/*/*"):
            print(filename[-2:])
            if ".c" != filename[-2:] and ".cpp" != filename[-4:]:
                continue
            print(filename)
            #if "ex"+str(args.exercise) not in os.path.basename(filename) or \
            #    (os.path.basename(filename).replace(".cpp", ".c")[-1] not in args.use_exercise and \
            #    os.path.basename(filename)[-2:].replace(".cpp", ".c") not in args.use_exercise):
            #    continue

            assert student_id in students, "履修者名簿にない学籍番号です．チェックしてください．"
            submit_id, _ = detect_exercise_num(os.path.basename(filename))
            print(submit_id, filename)

            #if submit_id not in args.use_exercise:
            #    continue

            if student_id not in zipfiles:
                zipfiles[student_id] = [(submit_id, filename)]
            else:
                zipfiles[student_id].append((submit_id, filename))

        #元のディレクトリ構造を用いる子がいるので
        for filename in glob.glob("tmp/" + student_id + "/*/*"):
            print(filename[-2:])
            if ".c" != filename[-2:] and ".cpp" != filename[-4:]:
                continue
